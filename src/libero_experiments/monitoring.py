...
class WarningController:
    """Turns risk into a boolean 'warning active' signal.

    Intended to operationalize the monitor as uncertainty / OOD detection.
    This does NOT change the model; it only gates a simple fallback policy
    in the environment loop (e.g., noop/abort).
    """
    tau: float
    patience: int = 1
    duration: int = 1
    ...
    def step(self, risk: float) -> tuple[bool, bool]:
        """Advance controller by one timestep.
