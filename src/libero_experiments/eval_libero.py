...
from libero_experiments.env_wrappers import DynamicsSpec, PerturbedEnv
...
def _failure_event_to_dict(failure_event) -> Optional[dict]:
    if failure_event is None:
        return None
    if hasattr(failure_event, "model_dump"):
        return failure_event.model_dump()
    if hasattr(failure_event, "__dict__"):
        return dict(failure_event.__dict__)
...
def eval_libero(cfg: RunConfig, intervention_config_path: str) -> EvalResult:
    ...
    if cfg.env.selected_task_ids:
        task_ids = list(cfg.env.selected_task_ids)
        bad_task_ids = [task_id for task_id in task_ids if task_id < 0 or task_id >= num_tasks_in_suite]
        if bad_task_ids:
            raise ValueError(
                f"selected_task_ids contains out-of-range task ids for {cfg.env.task_suite_name}: {bad_task_ids}"
            )
    else:
    ...
    for task_id in tqdm.tqdm(task_ids):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.env.num_trials_per_task)):
            ...
            if cfg.monitor.enabled and cfg.monitor.nearmiss.enabled:
                task_description, nearmiss_specs = sample_nearmiss_variant(
                    task_description, cfg.monitor.nearmiss, rng
                )
                perturbation = nearmiss_specs if nearmiss_specs else None
